...
contains
  ...
  subroutine read_ace_table(i_table, i_listing)
      ...
      if (.not. file_exists) then
        print *, "ACE library '" // trim(filename) // "' does not exist!"
        stop
      elseif (readable(1:3) == 'NO') then
        print *, "ACE library '" // trim(filename) // "' is not readable! &
        &Change file permissions with chmod command."
        stop
      end if

      ! display message
      print *, "Loading ACE cross section table: " // listing % name
  ...
  subroutine read_unr_res(nuc)
    ...
    if (nuc % urr_data % inelastic_flag > 0) then
      ...
      if (nuc % urr_inelastic == NONE) then
        print *, "Could not find inelastic reaction specified on " &
    ...
    if (any(nuc % urr_data % prob < ZERO)) then
       print *, "Negative value(s) found on probability table for nuclide " &
           // nuc % name
    end if
...
!===============================================================================
  ...
  subroutine read_thermal_data(table)
    ...
    if (table % secondary_mode /= SAB_SECONDARY_EQUAL .and. &
      ...
      print *, "Unsupported secondary mode on S(a,b) table " // &
